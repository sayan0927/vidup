...
public interface PlaylistVideoRepository extends JpaRepository<PlaylistVideo, PlaylistVideoPrimaryKey> {
     ...
     *                                              group by play.id
     */
    @Query(" SElECT new com.example.videostreamingcore.core.View_DTO.PlaylistDTO(p  ,COUNT(record.primaryKey.playlist)) FROM Playlist p LEFT JOIN PlaylistVideo record ON p=record.primaryKey.playlist WHERE p in (:playlists) GROUP BY p ORDER BY p.id")
